DB_mixing_organic<-function(dataset, initPL, initCT){
  ...
  #2. Estimated model parameters from calibration phase
  dataset$Vmax_glucose<-dataset$Plesne*initPL[["Vmaxg"]]+dataset$Certovo*initCT[["Vmaxg"]]
  dataset$Vmax_DOC<-dataset$Plesne*initPL[["Vmax"]]+dataset$Certovo*initCT[["Vmax"]]
  dataset$Km_glucose<-dataset$Plesne*initPL[["Kmg"]]+dataset$Certovo*initCT[["Kmg"]]
  dataset$Km_DOC<-dataset$Plesne*initPL[["Km"]]+dataset$Certovo*initCT[["Km"]]
  dataset$Ac_glucose<-dataset$Plesne*initPL[["Ac_glucose"]]+dataset$Certovo*initCT[["Ac_glucose"]]
  dataset$Ac_DOC<-dataset$Plesne*initPL[["Ac_DOC"]]+dataset$Certovo*initCT[["Ac_DOC"]]
  dataset$mr<-dataset$Plesne*initPL[["mr"]]+dataset$Certovo*initCT[["mr"]]
  dataset$f<-dataset$Plesne*initPL[["f"]]+dataset$Certovo*initCT[["f"]]
  dataset$Yu<-dataset$Plesne*initPL[["Yu"]]+dataset$Certovo*initCT[["Yu"]]
  dataset$fr<-dataset$Plesne*initPL[["fr"]]+dataset$Certovo*initCT[["fr"]]
  dataset$fs<-dataset$Plesne*initPL[["fs"]]+dataset$Certovo*initCT[["fs"]]
  #3. Rename "horizont"
  ...
  epar<-function(unlabelled, labelled){
    ...
    cost<-function(x){
      ...
      #model parameters
      mparl<-as.numeric(unlabelled[1, c("Vmax_glucose", "Vmax_DOC", "Km_glucose", "Km_DOC",
                                          "Ac_glucose", "Ac_DOC", "mr", "f", "Yu", "fs", "fr") ])
      names(mparl)<-c("Vmax_glucose", "Vmax_DOC", "Km_glucose", "Km_DOC",
                      "Ac_glucose", "Ac_DOC", "mr", "f", "Yu", "fs", "fr")
  ...
  epar_out<-foreach(i=6:10, .combine=list, .multicombine = TRUE,
  ...
  pars_all$horizon<-rep("Organic soil", times=5)
  pars_all$id<-seq(6,10)
  ...
  good_out<-foreach(i=6:10, .combine=list, .multicombine = TRUE,
                    .packages=c("FME", "dplyr", "DEoptim", "reshape")) %dopar% {

                      good(x=as.numeric(pars_all[pars_all$id==i, "Rinit"]),
  ...
  Gfit_treatments$N<-rep(5, times=nrow(Gfit_treatments))
  ...
  Gfit_labels$N<-rep(5, times=nrow(Gfit_labels))
  ...
  simul_out<-simul(x=as.numeric(pars_all[pars_all$id==6, "Rinit"]),
                   unlabelled = dataset[dataset$id==6, ],
                   labelled = dataset[dataset$id==16, ])

  #All other simulations are run and appended to existing "simul_out"
  for(i in 7:10){

    simul_out<-rbind(simul_out, simul(x=as.numeric(pars_all[pars_all$id==i, "Rinit"]),
                                      unlabelled = dataset[dataset$id==i, ],
                                      labelled = dataset[dataset$id==i+10, ]))
  }
