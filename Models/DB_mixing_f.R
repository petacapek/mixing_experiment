DB_mixing_f<-function(dataset, fractionsPL, fractionsCT, initPL, initCT, minpar, maxpar, f_litter, f_organic){
  ...
  #4. Add estimates of f parameter to the dataset
  dataset$f<-ifelse(dataset$horizon=="Litter", f_litter, f_organic)
  ...
  parnames<-c("Vmax_glucose", "Vmax_DOC", 
              "Km_glucose", "Km_DOC", 
              "Ac_glucose", "Ac_DOC",
              "mr", "Yu", "Rinit", "Ratm_init")#fr, fs and f will be supplied additionally
  ...
  epar<-function(unlabelled, labelled){
    ...
    cost<-function(x){
      ...
      #model parameters
      mparl<-c(par[1:8], "fr"=as.numeric(unlabelled[1, "fr"]), "fs"=as.numeric(unlabelled[1, "fs"]),
               "f"=as.numeric(unlabelled[1, "f"]))
  ...
  good_out<-foreach(i=seq(1:10), .combine=list, .multicombine = TRUE,
                    .packages=c("FME", "dplyr", "DEoptim", "reshape")) %dopar% {
  ...
  Yhat_treatments<-as.data.frame(rbind(good_out[[1]]$Yhat, good_out[[2]]$Yhat,
                                       ...
                                       good_out[[5]]$Yhat, good_out[[6]]$Yhat,
  ...
  Gfit_treatments$N<-rep(103, times=nrow(Gfit_treatments))
  ...
  Gfit_labels$N<-rep(103, times=nrow(Gfit_labels))
  ...
  Gfit_horizon$N<-rep(103, times=nrow(Gfit_horizon))
  ...
  Gfit_each$N<-rep(103, times=nrow(Gfit_each))
