Mend_py<-function(dataset, py_pars){
  ...
  MEND_model<-function(time, state, pars){
    with(as.list(c(state, pars)),{
      ...
      #Fluxes:
        ...
        #RGg - growth respiration - glucose
        #RMg - maintnance respiration - glucose
        #RGs - growth respiration - DOC
        #RMs - maintnance respiration - DOC
      #Scaling factors:
      ...
      #Model parameters:
        ...
        #CUE_g - carbon use efficiency in respect to glucose
        #CUE_s - carbon use efficiency in respect to DOC
        #mr - maintnance rate constant
        #kec - chloroform labile part of microbial biomass
      ...
      #States
      dMBC_12C<-Cu_glucose*(1-Gatm)+Cu_DOC*(1-DOCatm)-RGg*(1-Gatm)-RMg*(1-Gatm)-RGs*(1-DOCatm)-RMs*(1-DOCatm)-mr*MBC_12C
      dMBC_13C<-Cu_glucose*Gatm+Cu_DOC*DOCatm-RGg*Gatm-RMg*Gatm-RGs*DOCatm-RMs*DOCatm-mr*MBC_13C
      dG_12C<--Cu_glucose*(1-Gatm)
      dG_13C<--Cu_glucose*Gatm
      dDOC_12C<--Cu_DOC*(1-DOCatm)+kec*mr*MBC_12C
      dDOC_13C<--Cu_DOC*DOCatm+kec*mr*MBC_13C
      dCres_12C<-(1-kec)*mr*MBC_12C
      dCres_13C<-(1-kec)*mr*MBC_13C
      dCO2_12C<-RGg*(1-Gatm)+RMg*(1-Gatm)+RGs*(1-DOCatm)+RMs*(1-DOCatm)
      dCO2_13C<-RGg*Gatm+RMg*Gatm+RGs*DOCatm+RMs*DOCatm+mr*MBC_13C
  ...
  good<-function(x){
    ...
    yhat_all<-as.data.frame(ode(y=c(MBC_12C=MBC_12Cinit, MBC_13C=MBC_13Cinit,
                                ...
                                parms=par, MEND_model, times=t_sampling))
  ...
  simul<-as.data.frame(ode(y=c(MBC_12C=MBC_12Cinit, MBC_13C=MBC_13Cinit,
                               ...
                               DOC_12C=DOC_12Cinit, DOC_13C=DOC_13Cinit,
                               Cres_12C=0, Cres_13C=0, CO2_12C=0, CO2_13C=0),
                           parms=py_pars, MEND_model, times=t_simul))
